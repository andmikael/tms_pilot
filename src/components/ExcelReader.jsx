        ...
        reader.onload = async (e) => {
            try {
                ...
                for (let i = 0; i < jsonData.length; i++) {
                    ...
                    // Poistetaan ylimääräiset välilyönnit ja varmistetaan, että tiedot ovat olemassa ja oikeassa muodossa
                    const postalCode = String(row[0]).trim();
                    const name = String(row[1]).trim();
                    const address = String(row[2]).trim();
                    const city = String(row[3]).trim();
                    let standardPickup = String(row[4]).trim().toLowerCase();
                    ...
                    if (!name || !address || !postalCode || !city) continue;
